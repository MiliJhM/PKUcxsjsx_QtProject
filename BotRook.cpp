...
BotRook::BotRook(qreal hpmax, qreal atk, qreal expgiven, QObject* parent, BotBase* chessboard_[][8]) :BotBase(hpmax, atk, expgiven, parent), chessboard() {
    for (int i = 0; i < 8; i++) {
        chessboard[i] = chessboard_[i];
    }
    pic = QPixmap("./Images/BotRook.png");
    setPixmap(pic);
    setScale(0.39);
}

bool BotRook::attackableCheck(int x1, int y1)
{
    bool ret = 0;
    if (x1 - posx == 0 || y1 - posy == 0) {
        ret = 1;
...
void BotRook::moveRound(int x, int y) {
